...
exports.createDemoContainer1 = klass => {
  return [
    container,
    klass,
    {
  ...
  return [
    ...
    klass,
    {
      render(tokens, idx) {
        ...
        if (token.nesting === 1) {
          return `<div class="sealui-customblock is-${klass}" ${_icon ? 'style="padding-left:3.5rem"' : ""}>${
            _icon ? '<div class="sealui-customblock__icon">' + _icon + "</div>" : ""
          }\n`;
        } else {
          return `</div>\n`;
        }
